...
SYSTEM_PROMPT = (
    "You are a customer support quality analyst. You classify conversations step by step. Return ONLY valid JSON."
)

INTENT_DESCRIPTIONS = {
    "payment_issue": ("Problems with payments, payment methods, payment errors, or payment failures"),
    "technical_error": ("Placing orders, purchasing items, or registration problems"),
    "account_access": (
        "Creating, editing, deleting, or switching accounts; password recovery; updating personal information"
    ),
    "tariff_question": (
    ...
    "refund_request": ("Requesting refunds, checking refund status, refund policies, or canceling orders for a refund"),
    "other": (
...
SATISFACTION — judge by outcome (most are "satisfied" or "unsatisfied", rarely "neutral"):
- "satisfied": agent responded and tried to help. Even if generic or imperfect,
  if agent addressed the topic and client did not complain → "satisfied".
- "unsatisfied": agent clearly failed — wrong info, rude, ignored question,
  OR client expressed frustration/disappointment.
- "neutral": ONLY when truly ambiguous or cut short with no clear outcome.
  This should be rare (~15% of conversations).
...
Return JSON: {{"results": [
  {{"idx": 0, "reason": "<what is the client asking?>",
    "intent": "...", "satisfaction": "..."}}, ...]}}
...
- 5: Excellent. Clear, specific steps that fully resolve the issue.
  Example: exact navigation path, specific policy, concrete solution. (~25%)
- 4: Good. Helpful guidance, mostly addresses the issue.
  Example: relevant advice but misses some details. (~25%)
- 3: Adequate. Acknowledges problem, gives reasonable but generic response.
  Example: "I understand your concern, please try X". (~15%)
- 2: Poor. Unhelpful, misses the main question, or vague non-answers.
  Example: "Please contact support" without actually helping. (~25%)
- 1: Terrible. Actively rude, CLEARLY wrong info, or ignores customer.
  This is rare — a generic unhelpful answer is score 2, not 1. (~10%)
...
- "incorrect_info": agent stated something FACTUALLY WRONG (e.g., wrong price,
  wrong policy, wrong procedure steps). NOT incorrect_info: generic answers,
  template responses, incomplete info, or simply not knowing the answer
...
HIDDEN DISSATISFACTION — EXTREMELY RARE (expect less than 5%).
Set true ONLY if client says something like "fine, I'll figure it out myself"
or "okay whatever, thanks anyway" while their problem is clearly NOT solved.
In ALL other cases, set false. If unsure, set false.

{conversations}

Return JSON: {{"results": [
  {{"idx": 0, "reason": "<was the problem solved? how well?>",
    "quality_score": N, "agent_mistakes": [...],
...
SATISFACTION: "satisfied" if agent addressed the topic and client didn't complain,
"unsatisfied" if agent failed or client frustrated,
"neutral" ONLY if truly ambiguous (rare).

QUALITY SCORE (use the full range):
5=excellent specific resolution (~25%), 4=good helpful guidance (~25%),
3=adequate generic response (~15%), 2=poor unhelpful (~25%),
1=terrible rude/wrong (~10%).
...
HIDDEN DISSATISFACTION: EXTREMELY RARE. True only if client explicitly
gives up ("fine, I'll figure it out"). Otherwise false.

CONVERSATION:
{dialog_text}

Return JSON:
{{"reason": "<what is the client asking? was it resolved?>",
  "intent": "...", "satisfaction": "...", "quality_score": N,
  "agent_mistakes": [], "hidden_dissatisfaction": false}}
Return ONLY JSON."""
